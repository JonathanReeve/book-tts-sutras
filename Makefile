SRC = src/sutra_book.typ
PUBLIC = public

HTML_OUT = $(PUBLIC)/sutra_book.html
INO_PDF_OUT = $(PUBLIC)/sutra_book_ino.pdf
PRINT_PDF_OUT = $(PUBLIC)/sutra_book_print.pdf

FONT_PATH = $(shell nix-build --no-out-link '<nixpkgs>' -A noto-fonts-cjk-sans)/share/fonts/opentype

.PHONY: all html ino-pdf print-pdf clean

all: html ino-pdf print-pdf

html: $(HTML_OUT)

$(HTML_OUT): $(SRC)
	typst compile --font-path $(FONT_PATH) $(SRC) $(HTML_OUT) --features html --input show-ino-notation=false

ino-pdf: $(INO_PDF_OUT)

$(INO_PDF_OUT): $(SRC)
	typst compile --font-path $(FONT_PATH) $(SRC) $(INO_PDF_OUT) --input show-ino-notation=true

print-pdf: $(PRINT_PDF_OUT)

$(PRINT_PDF_OUT): $(SRC)
	typst compile --font-path $(FONT_PATH) $(SRC) $(PRINT_PDF_OUT) --input show-ino-notation=false

clean:
	rm -f $(HTML_OUT) $(INO_PDF_OUT) $(PRINT_PDF_OUT)
