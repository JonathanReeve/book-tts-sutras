...
var $author$project$Main$GotSutras = function (a) {
	return {$: 'GotSutras', a: a};
};
...
var $author$project$Main$init = _Utils_Tuple2(
	{sutras: $krisajenkins$remotedata$RemoteData$Loading},
	$elm$http$Http$get(
		{
			expect: $elm$http$Http$expectString($author$project$Main$GotSutras),
		...
		if (msg.$ === 'GotSutras') {
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						sutras: $krisajenkins$remotedata$RemoteData$fromResult(result)
					}),
		...
		while (true) {
			if (!rawLines.b) {
				...
				var $temp$rawLines = rawLinesTail,
					$temp$ast = A2($pablohirafuji$elm_markdown$Markdown$Block$incorporateLine, rawLine, ast);
...
var $author$project$Main$inoSymbols = function (content) {
	return A2(
		$elm$core$String$join,
		' ',
		A2(
...
var $author$project$Main$TranslationLine = F2(
	function (chineseRomaji, english) {
		return {chineseRomaji: chineseRomaji, english: english};
	});
var $author$project$Main$ChineseRomaji = F2(
	function (chinese, romaji) {
		return {chinese: chinese, romaji: romaji};
	});
...
var $author$project$Main$isChineseChar = function (_char) {
	return (_Utils_cmp(
		_char,
		_Utils_chr('\u4E00')) > -1) && (_Utils_cmp(
		_char,
		_Utils_chr('\u9FFF')) < 1);
};
...
var $author$project$Main$chineseRomajiParser = A2(
	$elm$parser$Parser$keeper,
	A2(
		$elm$parser$Parser$keeper,
		$elm$parser$Parser$succeed($author$project$Main$ChineseRomaji),
		$elm$parser$Parser$getChompedString(
			$elm$parser$Parser$chompWhile($author$project$Main$isChineseChar))),
...
var $author$project$Main$translationLineParser = A2(
	$elm$parser$Parser$keeper,
	A2(
		$elm$parser$Parser$keeper,
		$elm$parser$Parser$succeed($author$project$Main$TranslationLine),
		A2(
			$elm$parser$Parser$ignorer,
			A2(
				$elm$parser$Parser$ignorer,
										...
										$elm$parser$Parser$succeed(
											function (_new) {
												return $elm$parser$Parser$Loop(
													A2($elm$core$List$cons, _new, previous));
											}),
										$author$project$Main$chineseRomajiParser),
										$elm$parser$Parser$succeed(
...
var $author$project$Main$viewChineseRomaji = function (cr) {
	var _v0 = cr.romaji;
	if (_v0.$ === 'Just') {
		var romaji = _v0.a;
		return A3(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(romaji)
						]))
				]));
	} else {
		return $elm$html$Html$text(cr.chinese);
	}
};
var $author$project$Main$viewTranslationLine = function (line) {
	return A2(
		$elm$html$Html$div,
		_List_Nil,
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('chinese-romaji')
					]),
				A2($elm$core$List$map, $author$project$Main$viewChineseRomaji, line.chineseRomaji)),
				A2(
				$elm$html$Html$span,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(line.english)
					]))
			]));
};
var $author$project$Main$renderBlock = function (block) {
	var customInlineRenderer = function (inline) {
		if ((inline.$ === 'HtmlInline') && (inline.a === 'ino-annotation')) {
			var attrs = inline.b;
			var content = A2(
				$elm$core$Maybe$withDefault,
				'',
				A2(
					...
					$elm$core$List$head(
						A2(
							...
							attrs))));
			return A2($elm$core$String$startsWith, 'Note: ', content) ? A2(
				$elm$html$Html$span,
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(
						$author$project$Main$inoSymbols(content))
					]));
		} else {
			return A2($pablohirafuji$elm_markdown$Markdown$Inline$defaultHtml, $elm$core$Maybe$Nothing, inline);
		}
	};
	if (block.$ === 'Paragraph') {
		var content = block.a;
		var inlines = block.b;
		if (A2($elm$core$String$startsWith, '<trilingual-translation>', content)) {
			var translationContent = A2(
				$elm$core$String$dropRight,
				$elm$core$String$length('</trilingual-translation>'),
				A2(
			...
			var lines = A2(
				...
				A2(
					$elm$core$Basics$composeR,
					$elm$parser$Parser$run($author$project$Main$translationLineParser),
					$elm$core$Result$toMaybe),
				$elm$core$String$lines(translationContent));
			return _List_fromArray(
				[
					A2(
					$elm$html$Html$div,
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('trilingual-translation')
						]),
					A2($elm$core$List$map, $author$project$Main$viewTranslationLine, lines))
				]);
		} else {
			return A3(
				$pablohirafuji$elm_markdown$Markdown$Block$defaultHtml,
				$elm$core$Maybe$Just($author$project$Main$renderBlock),
				$elm$core$Maybe$Just(customInlineRenderer),
				block);
		}
	} else {
		return A3(
...
var $author$project$Main$view = function (model) {
	var preprocessInoAnnotations = function (text) {
		var parts = A2($elm$core$String$split, '@{', text);
		var _v1 = A2($elm$core$Debug$log, 'preprocessInoAnnotations input', text);
		if (parts.b) {
			var first = parts.a;
			var rest = parts.b;
			return _Utils_ap(
				...
				A2(
					...
					A2(
						$elm$core$List$map,
						function (s) {
							var _v3 = A2($elm$core$String$split, '}', s);
							if (_v3.b) {
								var content = _v3.a;
								var after = _v3.b;
								return '<ino-annotation content=\"' + (content + ('\" />' + A2($elm$core$String$join, '}', after)));
							} else {
								return s;
							}
						},
	...
	var customOptions = function (options) {
		return _Utils_update(
			options,
			{rawHtml: $pablohirafuji$elm_markdown$Markdown$Config$DontParse});
		...
		_List_fromArray(
			[
				...
				switch (_v0.$) {
					...
					default:
						var markdownText = _v0.a;
						return A2(
							$elm$html$Html$div,
							_List_Nil,
							A2(
								$elm$core$List$concatMap,
								$author$project$Main$renderBlock,
								A2(
									$pablohirafuji$elm_markdown$Markdown$Block$parse,
									$elm$core$Maybe$Just(customOptions),
									preprocessInoAnnotations(markdownText))));
				}
			}()
			]));
};
